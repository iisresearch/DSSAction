...
public class Story {
	...
	protected Set<Constraint> coveredConstraints = new HashSet<Constraint>();
	Set<String> coveredConcepts = new HashSet<String>();
	GateService annotator;
	...
	public Story(String storyFile, GateService annotator) {
		...
		try{
			DataInputStream in = new DataInputStream(this.getClass().getClassLoader().getResourceAsStream(storyFile));
